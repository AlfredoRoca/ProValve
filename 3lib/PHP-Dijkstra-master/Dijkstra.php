...
class Graph {
	...
	public function paths_from($from) {
		...
		$warnings = array(); // ARM añadido para detectar nodo origen aislado
		...
		while($Q->size() > 0) {
			...
			if (!isset($nodes[$u])) {
//				print "WARNING: '$u' is not found in the node list<br/>";
				$warnings[]=$u;
/*				print("<br/>--------- nodos no encontrados --------------<br/>");
				print_r($warnings);
				print("<br/>---------------------------------------------<br/>");
*/			} else {		
		...
		return array($dist, $previous, $warnings);  // ARM añadido warning al retorno de parametros
	}
